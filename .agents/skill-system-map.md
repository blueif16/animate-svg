...
| W1 storyboard | cue IDs + narration-beat intent + required visual; NO durations | `lesson-storyboard` | storyboard.md |
...
| W3b primitive gap-scan/build | reuse-or-build primitives (default REUSE); owns the intro card; wire the registry | `visual-discipline`, `kids-eye`, CAPABILITIES.md, catalog-digest.md, primitive-registry.json | primitive-gap-scan.md, src/shape-primitives/*, registry, primitive-checks/*.png |
...
- open — `teaching-action-vocabulary-gap` (kp1 post-mortem): 3 flaws; structural fix = a Teaching-Action vocabulary doc the gap-scan consumes (teaching-driven, not layout-driven). Candidate first real run through OPERATE.
