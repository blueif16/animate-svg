...
- **DECORATION NEVER INSIDE A MEASURED GROUP — the box must equal the true footprint.** A `measureProps(id)` `<g>` wraps ONLY the load-bearing teaching mark. Decoration — pulse/attention rings, glow halos, sparkles, eye-drawers, `<Breathe>`/shimmer wrappers used purely for liveliness — lives OUTSIDE the measured group, as its OWN `zone:"decoration"` element (or untagged). A ring nested inside a `labels`/`objects` `<g>` makes that element's `getBBox` engulf the ring, so the measured box no longer matches the thing it bounds → phantom or missed collisions (shipped failure: `aggregator-prompt` measured 516×400 from a ring inside the group, phantom-colliding the dots, while its honest label footprint was 960×60). And decoration is subordinate: it may sit BEHIND teaching content but must never cross/sit-on a teaching mark — the `decoration` zone exemption is for real backgrounds, not a license to park a ring on the dots; decoration that overlaps teaching is a defect to CUT, not exempt.
- **measure-id ≡ manifest-id is a BIJECTION (both directions).** Every `measureProps("x")` has exactly one `manifest.elements[].id === "x"`, AND every manifest element id is tagged somewhere in the scene. A `<g>` tagged with an id the manifest never declares falls through to `decoration` (silently collision-exempt); a manifest id never tagged in the scene is declared-but-never-measured. Both VOID detection (shipped failure: scene tagged one `"recap"` while the manifest declared `recap-beat-1-5/2-4/3-3` → join failure, recap collision detection void). `npm run lesson:bbox` reports a measured `decoration · …` element you never tagged as decoration, or a `… · declared only` element never measured — either is a bijection break.
- Before declaring Wave 4 done, run `npm run lesson:check -- --config lesson-data/<id>/pipeline.json --measured` and inspect:
  - `lesson-data/<id>/bbox-manifest.json` — fast `summary.collisionCount` must be `0`; the `measured` block's `summary.measuredCollisionCount` and `summary.gatesFailed` must be `0`/empty, OR every collision + failed gate explicitly justified — and a justification MUST quote the exact failing row from the gate JSON (element id + frame + measured value) before arguing it is by design. A justification that names an element or frame not present in the failing row is a contract breach, not a justification (shipped failure: a `bond-glyph@640 ratio 1.37` contrast FAIL "justified" as the recap sub-beat's intentional dimming — an element the gate never flagged). The linear path uses LINEAR progress and ~11 keyframes, so it misses easing-overshoot and between-keyframe overlaps; the measured pass catches them via real `getBBox()`. For a measured-only collision, open `out/<id>/measured-frames/f<frame>.png` and rule it a true overlap or a by-design adjacency in writing.
  - `out/<id>/bbox-overlay/f<frame>-bbox.png` — **the review surface for anything bbox-related** (`npm run lesson:bbox -- --config lesson-data/<id>/pipeline.json [--frames a,b]`). Draws each element's MEASURED box (solid, by zone) + DECLARED manifest box (dashed, when it diverges) with `id · zone · W×H` labels. NEVER hand a human a bare screenshot when a collision/size is in question — hand the boxed still. A >8px measured-vs-declared divergence is a manifest bug to fix.
