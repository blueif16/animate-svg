...
- **narration beat (INTENT, no copy)** — what the narration is *for* at this beat (follows from the teaching action: name the moment / model the target / invite the echo / recap). NEVER the actual words and NEVER a duration — copy is Wave 2b, timing is Wave 3.5. For a relation / bond / part–whole / retrieval beat, the intent is the COMPLETE utterance a teacher speaks — it names the WHOLE and its parts ("name that six splits into one and five"), never a stranded token ("say 一和五"). If pedagogy or any upstream draft phrased the discovery as a fragment, repair the intent to a complete utterance here — that is articulating intent, not re-deciding the teaching.
