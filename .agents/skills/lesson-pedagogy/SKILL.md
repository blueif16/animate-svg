...
**Acquisition carve-out (L2 utterances AND memorized facts).** This rule assumes the discovery is something the *picture* makes the child notice (a count, a relation, a transformation the picture is built to reveal). It **flips when the discovery is an acquisition target** — a thing the child must commit to memory and produce on demand (§8): a *word or sound* (an English "I'm", a pinyin tone) **or a *fact/bond* being drilled** (a number bond such as "1和5合成6", the conserved total of a 合 beat). For an acquisition target, *naming it fully IS the delivery, not leakage* — the child acquires a fact or sound by **hearing it said while seeing its instance**, so the **voice names the fact** and the **picture delivers the concrete instance** (the dots separating; who says it, when, to whom). Voicing "六分成一和五" or "一和五合起来是六" reinforces the bond; suppressing it (a silent merge, a self-imposed ban on "是六" / "还是六") *starves the very fact the beat exists to drill.* (This is §8's acquisition-vs-insight line: acquisition wants the voice to say it; insight wants the voice to withhold the aha.) The leakage rule still bites for any **insight, or a count the child should do,** that the picture is built to reveal — do not pre-name an aha the picture is staged to produce, and do not narrate a count the child is meant to perform. See §9.
...
**Breadth before depth — never starve a co-equal routine to over-drill the hard one.** The time-FLOOR is per-target, and applying it *only* to the hardest item is its own failure: it yields a lesson secretly about that one item with the others stapled on. When the knowledge point enumerates SEVERAL co-equal routines/targets (sibling routines, or a small set of equally-weighted target words), **each** earns its own staged delivery (a clean model where its target lands on its moment) **and** a retrieval in the recap — the child should leave feeling every one was taught, not one drilled and the rest name-checked. The `key_difficult` earns its EXTRA practice (e.g. a choral cue + a learner-response gap) **on top of** the others' baseline, never **instead of** it: do **not** give the hard target several dedicated cues while a co-equal routine is compressed to a single short cue. Budget the surplus so airtime across the enumerated routines stays roughly balanced — the hard one gets *somewhat* more, not several times more. When you write each cue's `reinforcement` line, sanity-check the WHOLE set: tally cues/seconds per routine and confirm none is starved. **Every beat that names a relation or retrieves a target speaks a COMPLETE, natural utterance** — the whole phrase a teacher would actually say aloud, never a telegraphic token, a verb stranded from its object, or a bare list (e.g. `"一和五，分成。"` and a recap of `"一和五。二和四。三和三。"` are fragments, not teaching). In a part–whole / relational lesson the utterance **names the whole it decomposes** (`"六分成一和五"`, `"一和五合起来是六"`) — a relation spoken without its whole is not the relation. A fragment is not a retrieval, and tacking one target's fragment onto the tail of another routine's recap line teaches nothing.
...
- **Learner-response gap (the wait-time).** A real planned beat: held silence of ≥3–5s after a "now you say it" prompt for the child to answer back before the reveal. Plan it for every acquisition target — it is the wait-time floor above, not an afterthought. It is **one reason** a lesson goes deliberately silent (others: letting an animation land, a breath, a dramatic beat) — all *intentional, typed silence*, the disciplined opposite of dead padding. No longer a "note where it belongs" hedge: downstream it is a real `gap: { seconds, reason: "learner-response" }` baked into the WAV (`docs/pipeline-architecture.md` §10) — the storyboard gives the echo its own beat, audio-captions authors the gap, and the silence costs nothing (locally generated, never a TTS call).
