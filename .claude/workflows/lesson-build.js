...
if (run('compose')) {
  ;[rCompose, rSketch] = await parallel([
    () => agent([
      ...
      `PREFLIGHT (before composing): run \`(cd ${REPO} && npm run registry:check-lesson -- ${P.primitiveGapScan} ${P.scene})\`. Every component the gap-scan/scene names MUST be a real registry id. If it flags a name, that component DOES NOT EXIST — it is a GAP, NOT a reuse. Do NOT grep/find the repo or read any OTHER lesson trying to locate it (it is not there — that hunt is the exact failure this gate prevents): either reach for the real registry primitive the beat needs, or hand-roll the mechanic inline from registered primitives, and record the phantom in \`issues\`. NEVER mount a component that isn't registered.`,
