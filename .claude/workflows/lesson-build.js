export const meta = {
  ...
  phases: [
    ...
    { title: 'Compose', detail: 'W4a composer (Complete<X>Lesson.tsx + scene + layout.ts + metadata-only manifest.ts; bed+SFX+captions wired; lesson:check) ∥ W4b sketch overlay' },
...
const P = {
  ...
  logs: `${data}/_logs`,
  voiceLog: `${data}/_logs/w3a-voice-asr.md`,             // W3a wave log; carries the machine-checked freeze record
...
if (run('wave3')) {
  ;[rVoice, rPrim, rSndAsset] = await parallel([
    ...
    () => agent([
      ...
      `STEP 3 — RECORD + FREEZE-CHECK the numbers before you declare freeze. Write the machine-readable freeze record into ${P.voiceLog}: a fenced \`\`\`freeze block of \`<cue-id>: <narrationFrames>\` lines plus \`total: <TotalNarrationFrames>\`, with EVERY value copied verbatim from the just-written ${P.clipsTs} (the AUDIO TRUTH — never from memory or an earlier run). THEN run the hard freeze gate: (cd ${REPO} && npm run lesson:freeze-check -- --config ${P.pipeline}). It diffs the accepted ${P.voiceLog} freeze record against the committed ${P.clipsTs} per-cue narrationFrames — and, when present, against out/${lessonId}/voice-clips.json + gemini-voice.json — and EXITS NON-ZERO with a per-cue diff table on ANY mismatch or a partial/EPERM-tainted write (a stale sibling left behind by a failed child-process write is exactly this class — self-scan C6). It MUST exit 0. If it fails: Clips.ts is the frozen truth — re-derive the stale artifact (the log record, and any stale voice-clips.json/gemini-voice.json) FROM Clips.ts and re-run; NEVER re-generate voice to make the numbers agree.`,
      'STEP 4 — FREEZE: once accepted, the per-cue CLIPS are canonical. The run does NOT advance to reconcile/composer until this audit passes AND audio-gate.json has pass:true (the three HARD checks: 0 drones AND 0 empty/short AND 0 dead-air) AND lesson:freeze-check exits 0. Truncation ADVISORIES do NOT block the freeze — they are surfaced (truncationAdvisories count) for a human spot-check, and the dedicated-TTS model is the structural prevention. Freeze as soon as the hard checks pass.',
      contract({ artifacts: [P.clipsTs, P.voiceWav, P.timingTs, `out/${lessonId}/audio-gate.json`], owns: [P.voiceClipsDir, P.clipsTs, P.voiceWav, P.geminiVoice, P.asr, P.timingTs, P.scriptCues, P.voiceLog], readScope: codeScope, note: 'scriptCues is owned ONLY for ASR-friendliness / anti-drone / anti-empty-clip re-rolls; voiceLog carries the freeze record lesson:freeze-check diffs against Clips.ts.' }),
...
const rReconcile = run('reconcile') ? await agent([
  ...
  `EMBED the reconciled output into ${REPO}/${P.timeline}: import ${camel}Clips, call reconcileClipTimeline(visualBudgets), and export the ${camel}Cues array AND the ${camel}VoiceClips array (the AudioLayer mounts one Sequence per clip). ALSO emit the cue-id UNION + throwing accessors so a wrong/stale id can NEVER silently render at frame 0: derive VISUAL_MOTION_SECONDS as an "as const" object (NO Record<string, number>) so its keys ARE the cue-id union keyof typeof VISUAL_MOTION_SECONDS (name it <Pascal>CueId — PascalCase of the lesson id — so a typo in scene/wrapper code is a COMPILE error); cross-check the OTHER direction (every budget key IS a frozen ${camel}Clips id, else THROW at import — reconcileClipTimeline already throws on a clip id with no budget); and export ${camel}CueAccessors = makeCueAccessors(reconciled.cues, <UPPER_SNAKE>_CUE_IDS) from ./_cues/cueAccessors — the scene + wrapper resolve EVERY cue id THROUGH these accessors (they THROW naming the unknown/stale id; there is NO c[id]?.startFrame ?? 0 fallback and NO Record<string, number> cue map anywhere). Audio, visuals and captions all read THIS. Do NOT create PADDED_CUE_DURATIONS_FRAMES, a continuous-WAV span, or any padding table. The brief **Length** is a HINT only — the true length is sum(cueFrames); accept drift. The module is EXACTLY this shape — it is COMPLETE; do NOT open any other lesson's timeline to "see the pattern" (cross-lesson reads are the READING-LAW breach):
  import { cueToCaption, reconcileClipTimeline, type CaptionCue, type VoiceClip } from "@studio/narration-kit";
  import { ${camel}Clips } from "./generated/${camel}Clips";
  import { makeCueAccessors } from "./_cues/cueAccessors";
  const FPS = 30; const TAIL_FRAMES = 9;
  const VISUAL_MOTION_SECONDS = { /* "<cue-id>": <seconds>, ONE entry per cue id IN ${camel}Clips (the frozen audio truth), from visual-design's motion-budget table. A design row whose beat W2b folded into a typed gap on a carrying cue gets NO entry (its dwell lives in that cue's gapFrames). */ } as const;
  export type <Pascal>CueId = keyof typeof VISUAL_MOTION_SECONDS;
  export const <UPPER_SNAKE>_CUE_IDS = Object.keys(VISUAL_MOTION_SECONDS) as <Pascal>CueId[];
  const FROZEN_CLIP_IDS = new Set(${camel}Clips.map((c) => c.id));
  for (const id of <UPPER_SNAKE>_CUE_IDS) if (!FROZEN_CLIP_IDS.has(id)) throw new Error("VISUAL_MOTION_SECONDS id not a frozen cue id: " + id);
  const reconciled = reconcileClipTimeline({ clips: ${camel}Clips, visualBudgets: VISUAL_MOTION_SECONDS, fps: FPS, tailFrames: TAIL_FRAMES });
  export const ${camel}Cues = reconciled.cues;
  export const ${camel}Duration: number = reconciled.durationFrames;
  export const ${camel}VoiceClips: VoiceClip[] = reconciled.voiceClips;
  export const ${camel}LessonCaptionCues: CaptionCue[] = reconciled.cues.map(cueToCaption);
  export const ${camel}CueAccessors = makeCueAccessors(reconciled.cues, <UPPER_SNAKE>_CUE_IDS);`,
...
if (run('compose')) {
  ;[rCompose, rSketch] = await parallel([
    () => agent([
      ...
      `INPUT: the reconciled ${camel}Cues in ${REPO}/${P.timeline} ; ${REPO}/${P.storyboard} (each cue's teaching action(s)) ; ${TEACH} (HONOR each move's \`requires\` — the layout/legibility constraints are binding: announce-topic ⇒ the title reads ALONE first, cast enters after (never overlay art on text); model-target-slow ⇒ target big+centered+nothing-on-top, held; spaced-recall ⇒ the live marker lands on the CURRENTLY-spoken item, never a stale row — and ANY spoken enumeration (count 1→N, per-number point/circle, per-word pulse) binds each step to that token's MEASURED ASR onset via the cue's \`tokenOnsets\` and the kit \`stepFramesFromOnsets\`/\`tokenOnsetFrame\` helper + the primitive's onset prop, NEVER a fixed step constant / even grid from the cue head; onsets absent ⇒ fall back to the constant AND emit a pipelineFinding) ; ${REPO}/${P.visualDesign} ; ${REPO}/${P.audioCaptions} ; ${REPO}/${P.audioCues} ; the verified primitives + test stills from W3b ; ${REPO}/${P.primitiveGapScan} ; ${CAP}.`,
      ...
      // self-check `lesson:check` BUNDLES the project, and the auto-discovery registry imports EVERY lesson's
      ...
      `SELF-CHECK before declaring done: (cd ${REPO} && npm run lesson:check -- --config ${P.pipeline}) — the measured pass is THE bbox check (no --measured flag needed). HARD gates that exit 1 (summary.gatesFailed): bbox-binding (the declared<=>measured bijection) + LUFS (master ~= -16 LUFS / TP <= -1). WARN (surface + fix-or-justify, do NOT fail the run): summary.measuredCollisionCount (overlap) + summary.captionIntrusionCount (teaching element in the caption ribbon). Any failed gate OR unjustified WARN requires a fix or an explicit written justification quoting the exact failing row (id + frame + value). Then rerun the pedagogy discovery audit against the rendered still. If the still/contact path cannot run, you are NOT done — status="blocked" with the blocker named; never substitute the bbox JSON for the rendered stills.`,
      `REVIEW SURFACE — NEVER review a bare screenshot: (cd ${REPO} && npm run lesson:bbox -- --config ${P.pipeline}) draws every registered element's MEASURED box (id · zone · W×H; a measured id the manifest never declared draws RED) into ${out}/bbox-overlay/ — review/attach THOSE boxed stills for any collision/size question, not a plain frame.`,
