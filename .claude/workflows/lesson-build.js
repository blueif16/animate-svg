...
const rStory = run('story') ? await agent([
  discipline(),
  'W1 — STORYBOARD. Define the cue IDs, the narration beat per cue, and the required visual per cue. NO duration estimates anywhere (timing is decided only by W3.5).',
  `SKILLS: ${SK.storyboard}`,
  `INPUT: ${REPO}/${P.pedagogy} (each cue must carry a discovery) ; ${REPO}/${P.brief}.`,
  `OUTPUT ARTIFACT: ${REPO}/${P.storyboard} — ordered cues (stable kebab ids), narration beat intent, required visual. The cue ids + order DEFINE the spine that voice / visuals / timing all bind to.`,
...
const rVdesign = run('design') ? await agent([
  ...
  `INPUT: ${REPO}/${P.storyboard} ; ${REPO}/${P.pedagogy} ; ${CAP} (reuse vocabulary).`,
...
if (run('compose')) {
  ;[rCompose, rSketch] = await parallel([
    () => agent([
      ...
      `INPUT: the reconciled ${camel}Cues in ${REPO}/${P.timeline} ; ${REPO}/${P.visualDesign} ; ${REPO}/${P.audioCaptions} ; ${REPO}/${P.audioCues} ; the verified primitives + test stills from W3b ; ${REPO}/${P.primitiveGapScan} ; ${CAP}.`,
