...
if (run('wave3')) {
  ;[rVoice, rPrim, rSndAsset] = await parallel([
    ...
    () => agent([
      ...
      `  THEN run the deterministic AUDIO GATE explicitly (it does NOT run inside lesson:voice — you MUST invoke it here, before freeze): (cd ${REPO} && npm run lesson:audio-gate -- --config ${P.pipeline}). It writes out/${lessonId}/audio-gate.json with FOUR pure-deterministic checks: 🔴 held-vowel DRONE, 🔴 EMPTY/SHORT clip (the TTS silently produced near-silence for a cue that has a phrase — catastrophic on a climax, invisible to drone/dead-air), 🔴 TRUNCATED clip (a NON-empty clip that stops mid-phrase — the model spoke only the first few tokens then cut off; detected by per-cue transcript-coverage vs the expected line AND by seconds-per-token far below this lesson's own cohort median, so it is language-general with no hard-coded rate), 🟡 untrimmed dead-air. \`pass:false\` means at least one check failed; the report's \`truncation\` block lists per-cue coverage + s/char + which signal tripped.`,
      'STEP 2 — VERIFY (do not trust): read gemini-voice.json transcriptText vs the script; walk per-cue matchScore + asrText in the ASR module + the audio-gate report (out/<id>/audio-gate.json, incl. its `truncation` block). A 🔴 DRONE flag means a cue authored an intra-cue pause as in-text dots ("I\'m…… Sam") — Gemini holds the vowel into a ~5s drone. A 🔴 EMPTY/SHORT flag means the TTS silently FAILED to render that cue (e.g. a 1-frame climax clip) — the cue plays SILENT and the lesson reads as "voice out of sync". A 🔴 TRUNCATED flag means the TTS produced a non-empty clip that CUT OFF mid-phrase (it spoke only the first few tokens) and that half-line would otherwise FREEZE silently — matchScore is informational and never caught this. FIX ANY: edit that cue\'s text in script-cues.json (a drone → typed gap/sub-beat, never in-text dots; an empty clip → reword the phrase the TTS choked on; a truncation → RE-ROLL the affected cue, and if it keeps truncating, shorten or split the cue\'s line so each utterance fits the generator\'s per-clip budget) and RE-ROLL voice for the affected cue(s) — repeat up to a SMALL N (≈3) re-rolls per cue, re-running the gate each time, until matchScore is acceptable AND audio-gate.json reports pass:true (every check, including 0 truncated). The re-roll only RECOVERS reliably when the generation path is the fixed dedicated-TTS model — treat that as guidance, not a hard precondition. If a cue is STILL truncated after N attempts, do NOT freeze it silently: either BLOCK (status="blocked", name the cue + its coverage/s-char) or record an EXPLICIT written justification in _logs/voice.md for why that specific cut is acceptable. Apply any ASR risk W2b flagged, or record a reasoned ignore.',
      'STEP 3 — FREEZE: once accepted, the per-cue CLIPS are canonical. The run does NOT advance to reconcile/composer until this audit passes AND audio-gate.json has pass:true (INCLUDING the truncation check — zero cues truncated, or an explicit per-cue justification recorded). NEVER freeze a clip that cuts off mid-phrase.',
      ...
      'RETURN accepted=true only when every cue\'s matchScore is acceptable AND audio-gate.json reports pass:true (0 drones AND 0 empty/short AND 0 truncated clips, or an explicit per-cue justification recorded for any remaining truncation); include the per-cue cueAudit (id, matchScore, durationFrames, reRolled, truncated).',
